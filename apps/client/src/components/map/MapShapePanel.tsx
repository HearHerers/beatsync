...
export const MapShapePanel = ({ canMutate }: MapShapePanelProps) => {
  ...
  // flight (silent until it lands); local "idle" = culled or failed.
  const localPlayback = useLocalZonePlayback();
  const serverIsPlaying = playlist?.playbackState.type === "playing";
  const localState = shape ? (localPlayback.get(shape.id) ?? "idle") : "idle";
        ...
        <div className="flex items-center justify-between gap-2 border-b border-neutral-800/50 px-4 py-3">
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2 text-[11px] text-neutral-500">
              ...
              {/* Local playback status — only meaningful while the server says
                  this zone is playing. "loading" = sound is coming once the
                  download/decode lands; "idle" = silent here (out of range,
                  or the load failed — see the toast). */}
              {serverIsPlaying && localState === "loading" && (
